...
class setMenuClass {
  ...
  activateSubmenu(link) {
    ...
    }, 500); //setTimeout moet hier 2x zo lang zijn als bovenstaande
  ...
  getClickLevel(link) {
    if (link.classList.contains('level_1')) return 1;else if (link.classList.contains('level_2')) return 2;else if (link.classList.contains('level_3')) return 3;else {
  ...
  setCurrentLevel(link) {
    ...
    if (this.getClickLevel(link) != false) {
      if (linkLevel === this.currentLevel) this.currentLevel++;else this.currentLevel--;
    }
...
class setMenuClassDesktop extends setMenuClass {
  ...
  hasDropdown(el) {
    if (el.querySelectorAll('ul')) return true;else return false;
